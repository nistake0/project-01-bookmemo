...
- 🔄 **Phase 9**: 新機能追加プロジェクト（計画中）
  - **優先度1**: メモOCR機能の実装（カメラ起動・テキストスキャン・自動入力）
  - **優先度2**: 本番デプロイ環境の構築（GitHub Pages・本番Firebase）
  - **優先度3**: 検索機能の改善（書籍・メモ検索の分離・タブ切り替え）
  - **優先度3**: PWA化（Service Worker・オフライン対応・インストール機能）
  - **優先度4**: タグ編集機能の実装（編集・削除・一括操作）
