...
- 書籍追加・編集時のタグ入力欄には、**Google Books APIの`categories`（カテゴリ情報）をデフォルトで自動セット**する。
  - 取得できない場合はopenBDの`subject`や`ndc`も利用。
  - 自動セットされたタグは、ユーザーが自由に編集・削除・追加できる。
- タグ入力欄は**MUIのAutocomplete（multiple）**を利用し、
  - ユーザーが過去に登録したタグ履歴をFirestore等から取得し、サジェスト・補完候補として表示。
  - 入力途中で候補が絞り込まれ、クリックやEnterで選択できる。
  - 新規タグも手入力で追加可能。
