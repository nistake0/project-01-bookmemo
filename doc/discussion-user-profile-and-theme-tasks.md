...
> **ステータス（2026-03-20）**: 本文のタスク **A / B / C はすべて実装済み**です。本ファイルは **設計経緯の記録**としての参照用であり、Cursor / 開発手順の**必須ドキュメントではありません**。現行のスキーマ・モジュールの一次情報は **`ARCHITECTURE.md`**、タスク管理は **`doc/bug-feature-memo.md`** を参照してください。

---

## ⚠️ 以下は歴史的決定事項（2025-01-31）

当時は「最優先目標」として扱っていたタスク定義です。**実装完了後の今日**は、拡張時の設計議論の参照に留めてください。
...
> **注（2026-03）**: 下表は執筆当時（2025-01-31）のスナップショットです。現在は `users/{uid}` に `profile` / `preferences` を保存し、`/settings`・PersonIcon 連携済みです。
