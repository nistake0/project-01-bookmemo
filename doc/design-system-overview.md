...
|------------|------|-----------------|---------------|--------------|
| backgroundDisplay | 背景表示ポリシー | full | solid-only | full |
| defaultBackgroundPresetId | テーマのデフォルト背景 | library-patterned | none | none |
| background.image | 背景画像（後方互換） | backgrounds/library.jpg | none | none |
| background.pattern | 背景パターン（後方互換） | backgrounds/library-pattern.svg | none | none |
...
- `doc/design-background-customization-20250320.md` - 背景カスタマイズ（ユーザー選択・複数候補）の設計
...
- `doc/discussion-user-profile-and-theme-tasks.md` - ユーザー設定・テーマタスク分離（実装完了済み・設計経緯の参照用）
