...
### 16. 新機能追加計画（2025-08-12追加）
- **優先度1: メモOCR機能の実装計画**
  - **目的**: メモ入力時の利便性向上、手動入力の負担軽減
  - **技術的実装**:
    - カメラAPI（getUserMedia）の活用
    - OCRライブラリ（Tesseract.js、Google Cloud Vision API等）の検討
    - メモ入力フォームへの統合（MemoAdd.jsx、MemoEditor.jsx）
    - スキャン結果の引用部分への自動挿入機能
  - **UX設計**:
    - カメラ起動ボタンの配置（引用入力エリア近く）
    - リアルタイムプレビュー機能
    - スキャン結果の編集・確認機能
    - エラーハンドリング（カメラ権限拒否、OCR失敗等）
  - **技術的考慮**:
    - モバイル対応（iOS Safari、Android Chrome）
    - プライバシー配慮（カメラ権限の適切な管理）
    - パフォーマンス最適化（画像処理の効率化）
    - オフライン対応の検討

- **優先度2: 本番デプロイ環境の構築計画**
  - **目的**: 本番環境でのサービス提供、開発・本番環境の分離
  - **技術的実装**:
    - 本番用Firebaseプロジェクトの作成・設定
    - GitHub Pagesでのデプロイ設定（gh-pages、GitHub Actions）
    - ルーティング競合回避（basename設定、既存ページとの共存）
    - 環境変数の本番設定（.env.production）
  - **デプロイ戦略**:
    - CI/CDパイプラインの構築（GitHub Actions）
    - 自動テスト実行（コミット時、プルリクエスト時）
    - 本番環境への自動デプロイ（mainブランチマージ時）
    - ロールバック機能の実装
  - **運用考慮**:
    - データベース移行（開発→本番）
    - セキュリティ設定の本番対応
...
- **優先度3: 検索機能の改善計画**
  - **目的**: 検索結果の明確化、ユーザビリティの向上
  - **技術的実装**:
    - 検索結果の種類別表示（書籍・メモ分離）
    - タブ切り替え機能の実装（書籍検索・メモ検索）
    - メモ検索結果の専用表示コンポーネント
    - 検索結果の詳細表示改善
  - **UX設計**:
    - 検索タイプの選択UI（書籍・メモ・両方）
    - 結果表示の最適化（メモの場合は引用部分の表示）
    - 検索結果の並び替え・フィルタリング機能
    - 検索履歴・お気に入り機能
  - **技術的考慮**:
    - パフォーマンス最適化（検索速度の向上）
...
- **優先度3: PWA化計画**
  - **目的**: ネイティブアプリのような体験提供、オフライン対応、インストール可能なWebアプリ
  - **技術的実装**:
...
- **優先度4: タグ編集機能の実装計画**
  - **目的**: タグ管理の利便性向上、データ品質の向上
  - **技術的実装**:
    - タグ編集・削除ダイアログの実装
    - 一括タグ操作機能（複数選択・一括編集）
    - タグ正規化・統合機能（類似タグの検出・統合）
    - タグ管理UIの改善
  - **UX設計**:
    - タグ編集モードの切り替え
    - ドラッグ&ドロップによる並び替え
