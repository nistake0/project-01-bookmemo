...
- [ ] **デザインシステム一元化の設計検討**（2026-01-11追加）
  - **目的**: フォントサイズ、ボタン・カードデザイン、レイアウト/配置の変更を一元的または比較的軽微な修正で対応可能にする設計を検討
  - **現状分析**: `doc/design-system-centralization-analysis.md`に詳細を記録
  - **検討事項**:
    1. **フォントサイズの完全一元化**: 現状60-70%がテーマ設定で管理可能。個別コンポーネントの`sx` propによる上書きを削減する設計を検討
    2. **ボタン・カードデザインの一元化**: 現状80-90%がテーマ設定で管理可能。個別上書きの最小化を検討
    3. **レイアウト/配置の一元化**: 現状20-30%のみ対応可能。アプローチの選択が必要:
       - アプローチA: テーマ設定での一元化（限定的）
       - アプローチB: 共通レイアウトコンポーネントの作成
       - アプローチC: デザイントークンの拡充（`layout.gridColumns`, `layout.cardGap`など）
  - **設計方針の決定が必要**: どこまで一元化するか、どのアプローチを採用するか
  - **関連ドキュメント**: `doc/design-system-centralization-analysis.md`
