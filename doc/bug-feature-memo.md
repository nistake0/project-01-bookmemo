...
- [ ] **UIデザイン改善プロジェクト** 🔄 **計画策定完了・実装待ち**
  ...
  - **実施計画**（Phase 1から順次実施）:
    - **Phase 1: 基盤整備**（高優先度）:
      1. Task 1.1: テーマ設定の一元化（推定1-2時間）
      2. Task 1.2: ハードコードされた色のテーマ移行（推定2-3時間）
      3. Task 1.3: CSSファイルの削除・移行（推定2-3時間）
    - **Phase 2: リファクタリング容易性向上**（中優先度）:
      4. Task 2.1: テストコードのデザイン独立性向上（推定4-6時間）
      5. Task 2.2: デザイントークンの整備（推定2-3時間）
    - **Phase 3: 長期改善**（低優先度）:
      6. Task 3.1: コンポーネントスタイルの重複排除（推定3-4時間）
  - **主な問題点**:
    - テーマ設定が2箇所に分散（`main.jsx`は未使用、`appTheme.js`が使用中）
    - ハードコードされた色が多数存在（`PageHeader`など）
    - CSSファイルとクラス名ベーススタイリングの混在（`TagStats.css`, `Stats.css`）
    - テストコードが実装文字列に依存（`.cursorrules`方針と矛盾）
