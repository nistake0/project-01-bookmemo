...
タスク・優先度・コードレビュー残件の**一次ソース**。`.cursorrules` の開発開始手順と整合させる。

## 開発セッション開始時チェックリスト

開発を始める前に以下を確認する（旧 `doc/development-startup-prompts.md` の必須運用はここへ集約）。

1. **日付** — 日報を書く場合は、記載日が現在日付と一致しているか確認する
2. **本ファイル** — 直下の「🔥 最優先タスク」と未完了のチェックボックスを読む
3. **設計** — `ARCHITECTURE.md` でデータモデル・テーマ・モジュール構成を把握する
4. **テスト** — ユニット: `npm run test:unit`。E2E: 先に `npm run dev`、続けて `npm run test:e2e`
5. **技術制約（Jest × Vite）** — `import.meta.env` をテストから直接使わない。外部 API 依存のユニットテストは簡略化・スキップや Cypress で代替する方針（`logger.js` は `process.env` 経由で本番判定）
6. **レビュー観点** — 新機能前に `doc/code-review-20260117.md` を確認する
...
#### 🎯 ユーザー設定・テーマ・プロフィール（2025-01-31 完了）

設計の経緯・タスク A/B/C の定義は **`doc/discussion-user-profile-and-theme-tasks.md`**（参照用・**必須ドキュメントではない**）。

| タスク | 内容 | 依存 | 推定工数 |
|--------|------|------|---------|
| **A. ユーザー設定の基盤整備** | Firestore スキーマ、useUserSettings フック、設定画面の土台、ボトムナビルート | なし | 1〜2日 |
| **B. テーマ選択機能** | テーマプリセット、ファクトリ、ThemeProvider 動的適用、選択UI | A | 3〜5日 |
...
- [ ] **優先度: 中の改善項目**:
  ...
  - [ ] 戻る操作の不具合調査と修正
- **確認タイミング**: 開発開始時、新機能追加前、定期的なコードレビュー時
- **参考**: `.cursorrules`の「コードレビュー優先事項」セクション

#### 🆕 新規UX改善タスク（2025-10-18追加）

以下の3つのタスクは、ユーザーからの要望として追加されました。設計議論から始め、段階的に実装します。

1. **PWA起動時ダイアログの無効化** ✅ **Phase 1完了**
   - [x] Phase 1: 起動時の自動表示を無効化（実装は残す）
   - [ ] Phase 2: ヘッダー等に手動で表示できるボタンを追加（将来実装）
   - [x] 対応内容: PWAInstallPromptの自動表示useEffectをコメントアウト
   - [x] 実装日: 2025-10-18
   - [x] コミット: `feat: PWA起動時ダイアログの自動表示を無効化`

2. **書籍詳細からの戻り先制御** ✅ **Phase 1-3完了**
   - [x] Phase 1: useNavigationフックの作成 ✅
   - [x] Phase 2: グローバルスワイプジェスチャ実装 ✅
   - [x] Phase 3: 各画面での適用 ✅
   - [x] 実装内容:
     - `src/hooks/useNavigation.js` - 統一ナビゲーションフック
     - `src/App.jsx` - グローバルスワイプジェスチャ（PWA時のみ）、タッチ開始位置判定
     - `src/components/MemoCard.jsx` - `data-allow-local-swipe`属性追加
     - `src/pages/TagSearch.jsx` - 検索結果の復元機能
     - `src/hooks/useSearch.js` - `setResults`公開
     - 競合回避機能実装
   - [x] 実装日: 2025-10-25
   - [x] コミット: 
     - `feat: 全画面統一ナビゲーション機能を実装`
     - `feat: 書籍詳細ページのカード内でもスワイプで戻れるように改善（タッチ開始位置判定に変更）`
     - `feat: 検索結果から書籍詳細に戻る際の検索結果復元機能を実装`
   - [x] 動作確認完了 ✅

3. **検索結果の永続化** ✅ **完了**
   - [x] 現状: 検索後に別画面へ行くと、戻った時に初期状態になる
   - [x] 要望: 最後の検索結果を表示したい
   - [x] 対応方法:
     - sessionStorageに検索結果を自動保存（30分TTL）
     - フック初期化時に自動復元
     - ブラウザ戻るボタンで復元
   - [x] 実装内容:
     - `src/utils/searchStorage.js` - sessionStorage管理ユーティリティ
     - `src/hooks/useSearch.js` - sessionStorage統合
     - `src/utils/searchStorage.test.js` - ユニットテスト8ケース
   - [x] 動作確認: 完了 ✅
   - [x] 実装日: 2025-10-25
   - [x] コミット: `feat: 検索結果のsessionStorage自動保存・復元機能を追加`
...
- [x] **UIデザイン改善プロジェクト** ✅ **完了** (2026-01-11)
  ...
  - **完了報告**: `doc/ui-design-improvement-project-completion.md`
  - **実施内容**:
    - **Phase 1: 基盤整備**（高優先度）✅:
    ...
    - **Phase 2: リファクタリング容易性向上**（中優先度）✅:
      4. Task 2.1: テストコードのデザイン独立性向上 ✅
      5. Task 2.2: デザイントークンの整備 ✅
    - **Phase 3: 長期改善**（低優先度）✅:
      6. Task 3.1: コンポーネントスタイルの重複排除 ✅ (2026-01-11完了)
  - **主な成果**:
    - テーマ設定の一元化により、デザイン変更が容易に
    - ハードコードされた色をテーマパレット参照に移行
    - CSSファイルを削除し、`sx` propに統一
    - テストコードがデザイン変更に影響されないように改善
    - 9箇所の重複スタイルを削除（Task 3.1）

- [ ] **戻る操作の不具合調査と修正**
  - メモ詳細や検索画面で戻る操作が期待通り動かないケースを再現・記録
  - `useNavigation`／`sessionStorage`復元ロジックを中心に原因を調査
...
- [x] **検索・タグページに「全文検索」タブを追加** ✅ **2025-10-18完了**
  - **⚠️ 重大な問題点（2025-09-28発覚）**:
    - デバウンス付きリアルタイム検索は、入力の度にFirebaseクエリを実行し続ける
    - モバイル環境での使用を考慮すると、データ通信量・コスト・パフォーマンスの観点で致命的
    - Firebase無料枠の制限（50,000回/日）を簡単に超過するリスク
    - ユーザーが「JavaScript」と入力すると、11文字×デバウンス = 最大11回のクエリ実行
  - **実装内容（2025-10-18完了）**:
    1. ✅ **検索実行ボタン必須** - リアルタイム検索を禁止
    2. ✅ **最小文字数制限** - 2文字以上で検索可能
    3. ✅ **LocalStorageキャッシュ** - 過去の検索結果をキャッシュ（最大100件、24時間保持）
    4. ✅ **レート制限** - 同一クエリは5秒以内に再実行不可（設定で簡単に変更可能）
    5. ✅ **適切なコンポーネント化** - 単一責任の原則に基づく設計
  - **実装ファイル**:
    - 設定: `src/config/fullTextSearchConfig.js`（調整可能なパラメータを集約）
    - フック: `src/hooks/useSearchCache.js`（キャッシュ管理）
    - フック: `src/hooks/useSearchRateLimit.js`（レート制限）
    - フック: `src/hooks/useFullTextSearch.js`（メインロジック）
    - UI: `src/components/search/FullTextSearch.jsx`（プレゼンテーション層）
    - ページ: `src/pages/TagSearch.jsx`（3タブ構成: 全文検索 / 詳細検索 / タグ管理）
  - **テストカバレッジ**: 包括的なテスト実装（config, hooks, component）
  - **品質指標**: 
    - Test Suites: 46 passed
    - Tests: 516 passed, 3 skipped
    - リンターエラー: なし
    - ビルド: 成功
  - **設計方針**:
    - 単一責任の原則を遵守（各ファイル1つの責務）
    - 設定の外部化（調整可能な値は定数ファイルに集約）
    - テスタビリティ重視（各機能を独立してテスト可能）
  - **所要時間**: 約5時間（設計・実装・テスト含む）

### 0. コード品質・リファクタリング（2025-01-21追加）

#### 完了済みタスク（2026-01-11）

- [x] **書籍データ編集機能の追加** ✅ **2026-01-11完了**
  - 書籍詳細画面に編集ダイアログを追加し、タイトル／著者／出版社／出版日／書影URLを更新可能にした。
  - openBD書影取得ボタンとプレビューを実装し、404時はダイアログ内にエラーメッセージを表示する。
  - 書籍情報の下部に小型の編集ボタンを配置し、主要情報の閲覧を妨げないUIに調整した。
  - `BookEditDialog.test.jsx` / `BookDetail.test.jsx` / `BookInfo.test.jsx` を整備し、書影取得とUI連携のユニットテストを追加した。
...
#### 優先度1（最優先）
- [ ] **書籍の同一性判定の改善（ISBN正規化）** 🔥 **高優先度**
  - [ ] ISBN正規化の強化（ハイフン除去、空白除去、大文字小文字統一）
  - [ ] 複合判定の導入（ISBN + タイトル + 著者での判定）
  - [ ] ファジーマッチング（タイトル・著者の類似度判定）
  - [ ] 出版社名の正規化
  - [ ] 重複チェック機能の精度向上
  - [ ] 既存データの正規化（マイグレーション）
- [ ] **未登録書籍の外部検索・追加機能**
...
- [x] **外部検索機能の実装** ✅ **2025-09-23完了**
  ...
  - [x] 外部検索ボタンUI改善（条件付き表示→常時表示）
  - [x] Google Books API設定問題の解決（環境変数アクセス方法修正）
  - [x] テストカバレッジ: 452/452テスト成功

#### 優先度5（UI改善）
- [x] **本一覧画面のUI改善** ✅ **2025-09-23完了**
  - [x] 重複ボタンの削除（「本を追加」ボタン）
  - [x] PWAインストールボタンの削除
  - [x] フッターメニューへの機能統一
  - [x] E2Eテストの更新（フッターメニュー使用）
  - [x] フッターメニューにdata-testid追加
  - [x] テストカバレッジ: 3/3テスト成功
- [x] **書籍詳細画面でのISBN表示機能** ✅ **2025-09-23完了**
  - [x] BookInfoコンポーネントにISBN表示を追加
  - [x] 出版日の下に「ISBN: {book.isbn}」を表示
  - [x] ISBNがある場合のみ表示（条件付きレンダリング）
  - [x] ユニットテストの更新（12/12 成功）
  - [x] ISBN表示・非表示のテストケース追加
...
#### 優先度1（最優先）
- [x] **メモランクの表現改善** ✅ **2025-11-01完了**
  - [x] ★なし: 「ランク未設定」→「ふつう」に変更（基準として設定）
  - [x] ★1-5の表現を段階的に改善（すべて「ふつう」より良い評価）
    - ★1: 「少し面白かった」
    - ★2: 「まあまあ面白かった」
    - ★3: 「面白かった」
    - ★4: 「とても面白かった」
    - ★5: 「神がかり的に面白い」
  - [x] ★なし（0）の場合も説明文を表示するように改善（MemoEditor、MemoAdd）
  - [x] 定数定義ファイル（`src/constants/memoRating.js`）の更新
  - [x] テストファイルの期待値を更新（`memoRating.test.js`、`MemoAdd.test.jsx`）
  - [x] テスト結果: 542 passed, 9 skipped（全テスト成功）
  - [x] 実装日: 2025-11-01
  - [x] 改善内容: 順序の不自然さを指摘され、すべてのランクが「ふつう」より良い評価になるように段階的な表現に修正

### UI/UX改善の優先タスク（2025-09-28追加）

#### 優先度1（最優先）🔥
- [x] **書籍ステータスに「中断」を追加** ✅ **2025-09-28完了**
  - [x] `BOOK_STATUS.SUSPENDED = 'suspended'`を追加
  - [x] ステータスラベル: '中断'、色: 'warning'（オレンジ色）
  - [x] ステータスの並び順: 積読 → 読書中 → **中断** → 再読中 → 読了
  - [x] 影響範囲: `src/constants/bookStatus.js`、`BookStatusChanger.jsx`、`BookList.jsx`、テストコード
  - [x] データ移行: 不要（既存データに影響なし）
  - [x] 実装時間: 約2時間
...
### 現状
- 検索結果画面や本一覧画面から、書籍を選んで詳細表示すると、そこから戻ることができない
- 特に検索一覧から詳細表示して戻るときは、直前の検索表示状態の画面に戻れない

### 要望
- 詳細表示から元の一覧画面や検索画面に戻れるようにする
- 検索一覧から詳細へ遷移した場合、戻るボタンで検索結果が復元されるようにする

### 技術的課題
- **ブラウザの戻るボタン**: 履歴スタックに基づいて戻るが、コンポーネントの状態は失われる
- **React Routerのlocation.state**: 遷移時に渡せるが、ブラウザの戻るボタンでは失われる
- **PWA対応**: PWAとして起動した場合、ブラウザの戻るボタンが存在しない
...
**従来の方針**:
- `location.state`で検索結果を渡す
- `sessionStorage`で保存/復元を試みる
- **問題**: `location.state`はブラウザの戻るボタンで失われる

**新しい方針**:
1. **グローバル状態管理の検討**
   - ZustandやReduxなどの状態管理ライブラリを導入
   - 検索結果をグローバルに保存
   - ブラウザの戻るボタンでも失われない

2. **useSearchフックの拡張**
   - 検索結果を`sessionStorage`に自動保存
   - フック初期化時に自動復元
   - コンポーネント側の実装を簡潔に

3. **実装レベル分け**
   - レベル1: PWAのスワイプジェスチャで対応（現在実装済み）
   - レベル2: `useSearch`フックで自動保存/復元
   - レベル3: グローバル状態管理ライブラリ導入

**次のステップ**:
- レベル2の実装を優先検討
- `useSearch`フックに`sessionStorage`自動保存/復元機能を追加
- 実装後に動作確認