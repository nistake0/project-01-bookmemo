...
- [ ] **検索・タグページに「全文検索」タブを追加**
  ...
  - **改善方針（未実装）**:
    ...
    2. **最小文字数制限** - 2文字以上で検索可能にする
    3. **検索履歴の活用** - 過去の検索結果をローカルキャッシュ
    4. **クライアントサイドフィルタリング** - 既に取得済みのデータから絞り込み
    5. **検索実行回数の制限** - 短時間での連続検索を防止
  - **仕様変更案**:
    - 全文検索タブ: テキストフィールド1つ + 「検索」ボタン（Enter押下も可）
    - 検索対象: 書籍タイトル、著者、メモ内容、タグ（すべてのステータス）
    - 最小入力文字数: 2文字
    - 検索結果キャッシュ: LocalStorageに保存（最大100件、24時間保持）
    - レート制限: 同一クエリは5秒以内に再実行不可
  - **新規コンポーネント**: `src/components/search/FullTextSearch.jsx`
  - **影響範囲**: `src/pages/TagSearch.jsx`（3タブ構成）
  - **実装時間**: 約3-4時間（キャッシュ・レート制限含む）
  - **優先度**: 最高（ただし、仕様確定後に実装）
